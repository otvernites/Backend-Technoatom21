...
class TicTacGame:
    def __init__(self, first_name="Player1", second_name="Player2", size=3):
        self.size = size
    ...
    def show_board(self):
        ...
        for i in range(count):
            print('+' + count * ('-' * 7 + '+'))
            board_str_i = "   |   ".join(str(x) for x in self.board[i])
            print('|   ' + board_str_i + '   |')
        print('+' + count * ('-' * 7 + '+'))
    ...
    def input_of_coordinates(self, sign):
        while True:
            try:
        ...
        for i in range(self.size ** 2):
            ...
            if res == 0:
                if i % 2 == 0:
                    ...
                    sign = 1
                self.input_of_coordinates(sign)
            elif res == 1:
    ...
    def diagonals(self, val):
        if all(self.board[i][i] == val for i in range(self.size)) or \
                all(self.board[i][self.size - 1 - i] == val for i in range(self.size)):
            return 1
        return 0
    ...
    def check_winner(self):
        ...
        for i in range(self.size):
            if all(elem == "X" for elem in self.board[i]) or \
                    all(elem == "X" for elem in self.column(i)) or \
                    self.diagonals("X"):
                result = 1
                return result
            if all(elem == "0" for elem in self.board[i]) or \
                    all(elem == "0" for elem in self.column(i)) or \
                    self.diagonals("0"):
                result = 2
                return result
        return result
